...
public class FirstHeuristic extends AlphaBetaSearch<Field, Movements, String>{
	HorseQueenGame game;
	private int expandedNodes;
	private int maxDepth;
	private String currentPlayer;
	
	public FirstHeuristic (HorseQueenGame game){
		super(game);
		this.game=game;
	}
	...
	public double eval(Field state, String player){
        ...
        Position[] jaque = state.getMovementsAvailableTo(state.getQueenPosition(player.charAt(0)));// hay que utilizar el del otro porque al mover se cambia el jugador actual
        ...
        if(state.getField()[state.getQueenPosition(player.charAt(0)).getX()][state.getQueenPosition(player.charAt(0)).getY()].getStack() < state.getField()[state.getQueenPosition(state.getOponent(player.charAt(0))).getX()][state.getQueenPosition(state.getOponent(player.charAt(0))).getY()].getStack()){
    	...
    	if(state.countHorses(state.getOponent(player.charAt(0))) < state.countHorses(player.charAt(0))){
			if (player.equals(currentPlayer)) return 15; 
			else return -15;
		}
